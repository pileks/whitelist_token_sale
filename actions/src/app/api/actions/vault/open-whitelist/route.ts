...
export const GET = (req: Request) => {
  const payload: ActionGetResponse = {
    ...
    title: "Open whitelist registration (vault version)",
    links: {
      actions: [
        {
          label: "Open whitelist",
          href: getUrlWithRequestOrigin(
            getActionQuery(actionUrls.vault.openWhitelist, params),
            req
          ),
          parameters: params,
        },
...
export const POST = async (req: Request) => {
  try {
    ...
    const payload = await createPostResponse({
      fields: {
        transaction,
        message: `You have opened whitelist registration for sale named "${saleName}".`,
      },
    });
